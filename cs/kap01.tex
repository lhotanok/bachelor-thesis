...
Pokud bychom se nespokojili s žádnou z dostupných datových sad, případně potřebovali data rozšířit a posbírat je přímo ze zdroje, využijeme proces zvaný \emph{web scraping}. V rámci tohoto procesu musíme analyzovat cílovou stránku z pohledu získávání a prezentace dat. S využitím vývojářským nástrojů ve webovém prohlížeči můžeme sledovat požadavky, které aplikace odesílá na svůj server a v mnoha případech se na toto interní API dokážeme napojit a data získat ve strukturované podobě. Typicky tímto způsobem obdržíme data ve formátu JSON. Pokud žádný fetch request pro získávání dat neobjevíme
